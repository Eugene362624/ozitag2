...
function App() {
    ...
    useEffect(() => {
        ...
        const openChild = (id) => {
            let i = id
            while (kids[i].dataset.level > 1){
                if (kids[i-1].dataset.level-1 <= kids[i].dataset.level) {
                    kids[i].removeAttribute('hidden')
                }
                i++
            }

            // [].forEach.call(document.getElementsByClassName('isOpen'), (e) => {
            //     for (let i = 0; i < kids.length; i++ ) {
            //         const next = 1+i >= kids.length ? i : 1+i
            //         if (kids[i].classList.contains('isOpen'))  {
            //             kids[next].removeAttribute('hidden')
            //             kids[next].classList.add('isOpen')
            //         }
            //     }
            // })
        }
